...
class BookOasisMateService:
    ...
    def _launch_maintenance_worker(self, paths, config, initial_status, sensitive_config=False):
        ...
        worker_script = Path(__file__).resolve().parents[1] / "workers" / "maintenance_worker.py"
        command = [
            sys.executable,
            str(worker_script),
            str(paths["config"]),
    ...
    def _start_database_migration_process(self, config):
        ...
        worker_script = Path(__file__).resolve().parents[1] / "workers" / "database_migration_worker.py"
        command = [
            sys.executable,
            str(worker_script),
            str(paths["config"]),
